...
- gson: `DB-JDBC×3` — a **false positive** (see below): gson has a `java.sql.Date/Time/Timestamp` TypeAdapter, so classifying by receiver type flags it as "DB" even though it does zero database I/O.
...
- petclinic-rest: 8 SQL methods → 16 edges over **6 real tables** (`PET, PETTYPE, SPECIALTY, USERS, VET_SPECIALTIES, VISIT`) → **3 clusters** — its JDBC repo variant exposes literal SQL that the JPA-only apps don't.
...
- **Receiver-type classification false-positives on *modeled* types.** gson gets `DB-JDBC×3` purely because it has a `java.sql.Date` TypeAdapter — it *models* the type without ever touching a database. "Reaches a `java.sql.*` type" ≠ "performs JDBC." Distinguishing use-of-subsystem from mention-of-type needs more than the receiver package (e.g. requiring a `Connection`/`Statement` in the call, not just the type).
- **Library vs application inverts `[BLOCK]`/`[EXPAND]`.** In an app, high fan-in = shared infrastructure to extract as a service. In a library (gson: 81 high-fan-in methods), high fan-in just means the *core public abstraction* — there's nothing to "rebuild." The fan-in signal is real but its *meaning* is mission-dependent, so the agent must interpret it, not apply a fixed rule.
